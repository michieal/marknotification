...
function notification_insert_custom_html($hook, $htype, $return, $params){
	 ...
	 if(!empty($return)){
		  ...
		  $frag->appendXml("<a href='{$link}' class='apop-notif-read' title='{$notif_title}'>{$notif_text}</a> ");

		  $notif_text = ossn_print('mark:notification:delete');
		  $notif_title = ossn_print('mark:notification:delete:title');
		  $link = ossn_site_url("action/mark/delete?guid={$nguid}", true);
		  $frag->appendXml("<a href='{$link}' class='apop-notif-delete' title='{$notif_title}'>{$notif_text}</a> ");
		  $frag->appendXML("<p>{$link}</p>");
		  $frag->appendXML("<hr style='color:#777;'/>");
